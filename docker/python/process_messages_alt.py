    ...
    def searchTarget(self,retDict):
        ...
        if targetKey:
            if targetKey in self.dictTargets: 
                ...
                print(  "trovato TARGET {0} nella cache con key = {1}".format(result["id"],  targetKey ))
                log.info(  "trovato TARGET {0} nella cache con key = {1}".format(result["id"], targetKey ))
                if retDict["type_event"] == "new_course":
                    setMessageLogStatus(self.host,self.port,self.user,self.password,self.database,retDict["idmessage_log"],1)
            else:
                ...
                if ret['success']:
                    ...
                    if( result ):
                        ...
                        if ret["success"]:
                            result = ret['result'] 
                            self.dictTargets[targetKey] = result
                            print(  "Creato TARGET {0} in VTE con cf_1545 = {1}".format(result["id"],  targetKey ))
                            log.info(  "Creato TARGET {0} in VTE con cf_1545 = {1}".format(result["id"],  targetKey ))
                            if retDict["type_event"] == "new_course":
                                setMessageLogStatus(self.host,self.port,self.user,self.password,self.database,retDict["idmessage_log"],3)
                        else:
